...
if len(models) == 0:
    raise FileNotFoundError("No model weights found in /models folder.")
...
def predict(image):
    ...
    total_weight = sum(ens_weights.get(n, 1.0) for n in models)
    for name in models:
        w = ens_weights.get(name, 1.0) / total_weight
        final_probs += w * model_probs[name]
    ...
    for name, r in model_outputs.items():
        details += (f"| {name} | {r['label']} | "
                    f"{r['benign']*100:.1f}% | "
                    f"{r['malignant']*100:.1f}% |\n")

    details += (f"\n## Ensemble Prediction: **{pred_label}**\n"
                f"Confidence: **{confidence*100:.2f}%**\n")

    if pred_label == "Malignant":
        details += "\n⚠️ Suggests malignant pattern. Consult a medical professional."
    else:
        details += "\n✅ Suggests benign pattern. Consult a medical professional."

    return pred_label, label_scores, details


# load all example images from examples/ folder
examples = []
if EXAMPLES_DIR.exists():
    for img_file in sorted(EXAMPLES_DIR.glob("*.png")):
...
# ── UI ────────────────────────────────────────────────────────────
with gr.Blocks(title="Breast Cancer Detection",
...
""")
    ...
    with gr.Row():
        with gr.Column():
            image_input = gr.Image(type="pil",
                                   ...
                                   height=320)
            run_btn = gr.Button("Analyse Image", variant="primary")

        with gr.Column():
            text_output  = gr.Textbox(label="Prediction", lines=1)
    ...
    run_btn.click(fn=predict,
                  inputs=image_input,
                  outputs=[text_output, label_output, details_output])
